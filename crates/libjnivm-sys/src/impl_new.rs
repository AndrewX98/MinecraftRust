...
use crate::state::{jvm_state, get_class_name_from_handle, alloc_object_fields, set_object_class};
use std::ffi::CStr;

fn tag_object_class(obj: jobject, clazz: jclass) {
    if clazz.is_null() { return; }
    if let Some(name) = get_class_name_from_handle(clazz) {
        set_object_class(obj, name);
    }
}
