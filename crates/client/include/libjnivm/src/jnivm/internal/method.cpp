            ...
            auto ccl =
                    std::find_if(cur->methods.begin(), cur->methods.end(),
                                            [&sname, &ssig](std::shared_ptr<Method> &namesp) {
                                                return namesp->_static == isStatic && namesp->name == sname && namesp->signature == ssig;
                                            });
...
template<class T> T jnivm::MDispatchBase2<T>::CallMethod(JNIEnv *env, jobject obj, jmethodID id, jvalue *param) {
    ...
    if (obj && mid && mid->nativehandle) {
        auto cl = JNITypes<std::shared_ptr<Class>>::JNICast(ENV::FromJNIEnv(env), env->GetObjectClass(obj));
        if (cl) clsname = cl->name.c_str();
    }
#ifdef JNI_TRACE
    fprintf(stderr, "TRACE: CM mid=%p nh=%p nat=%p obj=%p cls=%s meth=%s sig=%s\n",
        (void*)mid,
        (void*)(mid ? mid->nativehandle.get() : nullptr),
        (void*)(mid ? (void*)mid->native : nullptr),
        (void*)obj,
        clsname,
...
#ifdef JNI_TRACE
            fprintf(stderr, "TRACE: CallMethod caught: %s\n", e.what());
#endif
...
#ifdef JNI_TRACE
            fprintf(stderr, "TRACE: CallMethod caught unknown\n");
#endif
            auto cur = std::make_shared<Throwable>();
            cur->except = std::current_exception();
            (ENV::FromJNIEnv(env))->current_exception = cur;
    ...
    } else if (mid && mid->native) {
        auto sig = mid->signature;
        auto rparen = sig.find(')');
        size_t nparams = 0;
        bool has_fd = false;
        for (size_t i = 1; i < rparen; ) {
            nparams++;
            if (sig[i] == 'F' || sig[i] == 'D') has_fd = true;
            if (sig[i] == 'L') { while (i < rparen && sig[i] != ';') i++; i++; }
            else if (sig[i] == '[') { i += 2; }
            else { i++; }
        }
        if (has_fd) {
#ifdef JNI_TRACE
            fprintf(stderr, "TRACE: CallMethod(reg native) float/double params not supported for %s\n", sig.c_str());
#endif
            return defaultVal<T>(ENV::FromJNIEnv(env), sig);
        }
        void* fn = mid->native;
#ifdef JNI_TRACE
        fprintf(stderr, "TRACE: reg_native %s %s nparams=%zu fn=%p\n", mid->name.c_str(), sig.c_str(), nparams, fn);
#endif
        try {
            if (nparams == 0) {
                return ((T(*)(JNIEnv*, jobject))fn)(env, obj);
            } else if (nparams == 1) {
                return ((T(*)(JNIEnv*, jobject, intptr_t))fn)(env, obj, *(const intptr_t*)&param[0]);
            } else if (nparams == 2) {
                return ((T(*)(JNIEnv*, jobject, intptr_t, intptr_t))fn)(env, obj,
                    *(const intptr_t*)&param[0], *(const intptr_t*)&param[1]);
            } else if (nparams == 3) {
                return ((T(*)(JNIEnv*, jobject, intptr_t, intptr_t, intptr_t))fn)(env, obj,
                    *(const intptr_t*)&param[0], *(const intptr_t*)&param[1], *(const intptr_t*)&param[2]);
            } else if (nparams == 4) {
                return ((T(*)(JNIEnv*, jobject, intptr_t, intptr_t, intptr_t, intptr_t))fn)(env, obj,
                    *(const intptr_t*)&param[0], *(const intptr_t*)&param[1], *(const intptr_t*)&param[2], *(const intptr_t*)&param[3]);
            } else if (nparams == 5) {
                return ((T(*)(JNIEnv*, jobject, intptr_t, intptr_t, intptr_t, intptr_t, intptr_t))fn)(env, obj,
                    *(const intptr_t*)&param[0], *(const intptr_t*)&param[1], *(const intptr_t*)&param[2], *(const intptr_t*)&param[3], *(const intptr_t*)&param[4]);
            } else if (nparams == 6) {
                return ((T(*)(JNIEnv*, jobject, intptr_t, intptr_t, intptr_t, intptr_t, intptr_t, intptr_t))fn)(env, obj,
                    *(const intptr_t*)&param[0], *(const intptr_t*)&param[1], *(const intptr_t*)&param[2], *(const intptr_t*)&param[3], *(const intptr_t*)&param[4], *(const intptr_t*)&param[5]);
            } else {
#ifdef JNI_TRACE
                fprintf(stderr, "TRACE: reg_native %s nparams=%zu > 6, defaultVal\n", mid->name.c_str(), nparams);
#endif
                return defaultVal<T>(ENV::FromJNIEnv(env), sig);
            }
        } catch (const std::exception& e) {
#ifdef JNI_TRACE
            fprintf(stderr, "TRACE: CallMethod(reg native) caught: %s\n", e.what());
#endif
            auto cur = std::make_shared<Throwable>();
            cur->except = std::current_exception();
            (ENV::FromJNIEnv(env))->current_exception = cur;
...
#ifdef JNI_TRACE
            fprintf(stderr, "TRACE: CallMethod(reg native) caught unknown\n");
#endif
            auto cur = std::make_shared<Throwable>();
            cur->except = std::current_exception();
            (ENV::FromJNIEnv(env))->current_exception = cur;
...
template<class T> T jnivm::MDispatchBase2<T>::CallMethod(JNIEnv *env, jobject obj, jclass cl, jmethodID id, jvalue *param) {
    ...
    if(!id)
        LOG("JNIVM", "CallMethod field is null");
#endif
    const char* clsname_nv = "(null)";
    if (obj && mid && mid->nativehandle && cl) {
        auto clz = JNITypes<std::shared_ptr<Class>>::JNICast(ENV::FromJNIEnv(env), cl);
        if (clz) clsname_nv = clz->name.c_str();
    }
#ifdef JNI_TRACE
    fprintf(stderr, "TRACE: CM_NV mid=%p nh=%p nat=%p obj=%p cls=%s meth=%s sig=%s\n",
        (void*)mid,
        (void*)(mid ? mid->nativehandle.get() : nullptr),
        (void*)(mid ? (void*)mid->native : nullptr),
        (void*)obj,
...
#endif
    const char* clsname_st = "(null)";
    if (cl) clsname_st = cl->name.c_str();
#ifdef JNI_TRACE
    fprintf(stderr, "TRACE: CM_ST mid=%p nh=%p nat=%p cls=%s meth=%s sig=%s\n",
        (void*)mid,
        (void*)(mid ? mid->nativehandle.get() : nullptr),
        (void*)(mid ? (void*)mid->native : nullptr),
        clsname_st,
