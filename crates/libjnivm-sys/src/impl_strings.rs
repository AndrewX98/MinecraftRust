...
pub unsafe extern "C" fn jni_NewString(_env: *mut JNIEnv, unicode: *const jchar, len: jsize) -> jstring {
    ...
    // C++ builds an (empty) String object even for size == 0; only a null
    // pointer has no representation here.
    let slice = if len > 0 { std::slice::from_raw_parts(unicode, len as usize) } else { &[] };
...
pub unsafe extern "C" fn jni_GetStringLength(_env: *mut JNIEnv, s: jstring) -> jsize {
    ...
    // JNI length is the number of UTF-16 code units, not UTF-8 bytes.
    CStr::from_ptr(s as *const i8).to_string_lossy().encode_utf16().count() as jsize
}
...
pub unsafe extern "C" fn jni_NewStringUTF(_env: *mut JNIEnv, utf: *const i8) -> jstring {
    // C++ maps a null input to the empty string (never null).
    let owned = if utf.is_null() {
        String::new()
    } else {
...
pub unsafe extern "C" fn jni_GetStringRegion(_env: *mut JNIEnv, str: jstring, start: jsize, len: jsize, buf: *mut jchar) {
    if str.is_null() || buf.is_null() || len <= 0 { return; }
    let owned = CStr::from_ptr(str as *const i8).to_string_lossy();
    let mut out = buf;
    // C++ steps one character at a time; clamp instead of overflowing.
    for ch in owned.chars().skip(start.max(0) as usize).take(len as usize) {
        let mut tmp = [0u16; 2];
...
pub unsafe extern "C" fn jni_GetStringUTFRegion(_env: *mut JNIEnv, str: jstring, start: jsize, len: jsize, buf: *mut i8) {
    if str.is_null() || buf.is_null() || len <= 0 { return; }
    let owned = CStr::from_ptr(str as *const i8).to_string_lossy().into_owned();
    let mut off = 0usize;
    for ch in owned.chars().skip(start.max(0) as usize).take(len as usize) {
        let mut tmp = [0u8; 4];
        let enc = ch.encode_utf8(&mut tmp);
        std::ptr::copy_nonoverlapping(enc.as_ptr(), buf.add(off) as *mut u8, enc.len());
        off += enc.len();
    }
    // C++ always NUL-terminates the region.
    *buf.add(off) = 0;
}
