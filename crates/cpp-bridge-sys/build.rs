...
fn main() {
    let client_dir = PathBuf::from(std::env::var("CARGO_MANIFEST_DIR").unwrap()).join("../client");
    ...
    bridge.file(client_dir.join("src/capi.cpp"));
    bridge.compile("mcpelauncher-client-bridge");

    // --- Compile mcpelauncher-linker sources ---
    ...
    // --- Compile mcpelauncher-core sources ---
    ...
    // --- Compile manifest library sources ---
    ...
    for f in &[
        ...
        manifest_libs.file(client_dir.join("src/manifest_libs").join(f));
    }
    ...
    base64.cpp(true)
        .std("c++17")
        .flag_if_supported("-w");
    base64.include(&local_inc);
    base64.file(client_dir.join("src/manifest_libs/base64/base64.cpp"));
    ...
    client.cpp(true)
        .std("c++17")
        .flag_if_supported("-w");
    client.include(client_dir.join("src"));
    client.include(client_dir.join("src/manifest_headers"));
    client.define("EnableJNIVMGC", "1");
    client.include(local_inc.join("libjnivm"));
    client.include(local_inc.join("android-support-headers"));
    client.include(local_inc.join("game-window"));
    client.include(local_inc.join("logger"));
    client.include(local_inc.join("mcpelauncher-common"));
    client.include(local_inc.join("msa-daemon-client"));
    client.include(local_inc.join("cll-telemetry"));
    client.include(local_inc.join("file-util"));
    client.include(local_inc.join("mcpelauncher-core"));
    client.include(local_inc.join("sdl3"));
    client.include(local_inc.join("daemon-utils"));
    client.include(local_inc.join("simple-ipc"));
    client.include(local_inc.join("epoll-shim"));
    client.include(local_inc.join("properties-parser"));
    client.include(local_inc.join("mcpelauncher-errorwindow"));
    client.include(local_inc.join("linux-gamepad"));
    client.include(local_inc.join("file-picker"));
    client.include(local_inc.join("libc-shim"));
    if nlohmann_json_include.exists() {
        client.include(nlohmann_json_include);
    }
    client.include(client_dir.join("include"));
    ...
    client.file(client_dir.join("src/main_stubs.cpp"));
    ...
    println!("cargo:rerun-if-changed=../client/src/capi.cpp");
    println!("cargo:rerun-if-changed=../client/src/main_stubs.cpp");
    println!("cargo:rerun-if-changed=../client/src/mcpelauncher-linker/bionic/libc/bionic/bionic_call_ifunc_resolver.cpp");
    for f in &linker_files {
        println!("cargo:rerun-if-changed=../client/src/mcpelauncher-linker/{}", f);
    }
    for f in &[
        "src/hook.cpp",
        "src/mod_loader.cpp",
        ...
        println!("cargo:rerun-if-changed=../client/src/mcpelauncher-core/{}", f);
    }
    for f in &stub_files {
        println!("cargo:rerun-if-changed=../client/src/{}", f);
    }
    for entry in std::fs::read_dir(client_dir.join("src/jni")).unwrap() {
        let entry = entry.unwrap();
        let path = entry.path();
    ...
    println!("cargo:rerun-if-changed=../client/src/manifest_libs/base64/base64.cpp");
    for f in &[
        "common/connection_internal.cpp",
        "common/encoding/encodings.cpp",
        "common/encoding/encoding_json.cpp",
        ...
        println!("cargo:rerun-if-changed=../client/src/manifest_libs/simpleipc/{}", f);
    }
    println!("cargo:rerun-if-changed=../client/src/manifest_libs/daemon-utils/client/src/daemon_launcher.cpp");
    for f in &["src/service_client.cpp", "src/token.cpp"] {
        println!("cargo:rerun-if-changed=../client/src/manifest_libs/msa-daemon-client/{}", f);
    }
    for f in &[
        "src/event_manager.cpp",
        "src/configuration.cpp",
        ...
        println!("cargo:rerun-if-changed=../client/src/manifest_libs/cll-telemetry/{}", f);
    }
    for f in &[
        "src/gamepad.cpp",
        "src/gamepad_mapping.cpp",
        ...
        println!("cargo:rerun-if-changed=../client/src/manifest_libs/linux-gamepad/{}", f);
    }
    for f in &[
        "game_window_manager.cpp",
        "game_window_error_handler.cpp",
        ...
        println!("cargo:rerun-if-changed=../client/src/manifest_libs/gamewindow/{}", f);
    }
}
