...
#ifdef JNI_TRACE
    fprintf(stderr, "TRACE: Throw called, ex=%p\n", (void*)ex);
#endif
    auto except = JNITypes<std::shared_ptr<jnivm::Throwable>>::JNICast(ENV::FromJNIEnv(env), ex);
#ifdef JNI_TRACE
    fprintf(stderr, "TRACE: Throw JNICast done, except=%p current_exception was=%p\n", (void*)except.get(), (void*)(ENV::FromJNIEnv(env))->current_exception.get());
#endif
...
#ifdef JNI_TRACE
    fprintf(stderr, "TRACE: ThrowNew called, class=%p message=%s\n", (void*)c, message ? message : "(null)");
#endif
	try {
...
void ExceptionDescribe(JNIEnv *env) {
	fprintf(stderr, "ED: env=%p\n", (void*)env);
	if(!env) { fprintf(stderr, "ED: null env\n"); return; }
	fprintf(stderr, "ED: env->functions=%p\n", (void*)env->functions);
	if(!env->functions) { fprintf(stderr, "ED: null functions\n"); return; }
	fprintf(stderr, "ED: env->functions->GetVersion=%p\n", (void*)env->functions->GetVersion);
	fprintf(stderr, "ED: env->functions->reserved0=%p\n", (void*)env->functions->reserved0);
	if(!env->functions->reserved0) { fprintf(stderr, "ED: null reserved0\n"); return; }
	auto nenv = ENV::FromJNIEnv(env);
	fprintf(stderr, "ED: nenv=%p\n", (void*)nenv);
	if(!nenv) { fprintf(stderr, "ED: null nenv\n"); return; }
	fprintf(stderr, "ED: current_exception=%p\n", (void*)nenv->current_exception.get());
	if(nenv->current_exception) {
		fprintf(stderr, "ED: clearing exception\n");
		{ // Print class name of the exception object
			auto cl = nenv->current_exception->clazz.lock();
			if(cl) {
				fprintf(stderr, "ED: exception class: %s\n", cl->getName().c_str());
			} else {
				fprintf(stderr, "ED: exception class: (unknown)\n");
			}
		}
		if(nenv->current_exception->except) {
			try {
				std::rethrow_exception(nenv->current_exception->except);
			} catch (const std::exception& e) {
				fprintf(stderr, "ED: exception what: %s\n", e.what());
			} catch (...) {
				fprintf(stderr, "ED: unknown exception type\n");
			}
		}
		nenv->current_exception = nullptr;
	} else {
		fprintf(stderr, "ED: no pending exception\n");
	}
	fflush(stderr);
};
