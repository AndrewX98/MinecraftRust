...
fn main() {
    ...
    // Optional Mesa workaround: hide instanced-array extension strings.
    // Do NOT set this by default — Minecraft 1.21+ calls glVertexAttribDivisor /
    // glDraw*Instanced without null-checking eglGetProcAddress, so FakeEGL now
    // supplies real (or no-op) entry points. Users on Mesa 23.1 can still set
    // MESA_EXTENSION_OVERRIDE manually if needed.
    // (Previously we forced -GL_EXT_instanced_arrays here.)
